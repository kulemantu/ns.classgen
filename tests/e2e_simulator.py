...
async def test_api_chat(client, message, name):
    ...
    try:
        response = await client.post(API_URL, json={"message": message, "thread_id": thread_id}, timeout=45.0)
        ...
        if response.status_code == 200:
            ...
            print(f"[{name}] SUCCESS ({duration:.2f}s) | Reply Length: {reply_len} | PDF Generated: {has_pdf}")
...
async def main():
    ...
    async with httpx.AsyncClient() as client:
        # 1. Standard Case
        res, t1 = await test_api_chat(client, "I need a 45 min lesson on the water cycle for 5th graders.", "Standard")
