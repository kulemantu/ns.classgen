...
def test_log_session_no_supabase():
    # When Supabase is None, it should just print and return None
    with patch('utils.supabase', None):
        result = utils.log_session("thread_1", "user", "hello")
        assert result is None

def test_get_session_history_no_supabase():
    # When Supabase is None, it should return empty list
    with patch('utils.supabase', None):
        result = utils.get_session_history("thread_1")
        assert result == []

def test_log_session_with_supabase():
    mock_supabase = MagicMock()
    # Mocking the builder pattern: supabase.table().insert().execute()
    ...
    with patch('utils.supabase', mock_supabase):
        result = utils.log_session("thread_1", "user", "hello")
