...
class TestHomeworkApiTeacherField:
    """V4.2a — `GET /api/h/{code}` surfaces the teacher's name + slug when
    the homework code carries a `teacher_phone` linked to a registered
    teacher row. Lets the student-facing page show "Set by Mrs. Okafor"."""

    def setup_method(self):
        _mem_homework.clear()
        # Clear in-memory teachers table
    ...
    def test_response_includes_teacher_when_teacher_registered(self):
        from classgen.data.teachers import save_teacher

        save_teacher("+23412345678", name="Mrs. Okafor", country="Nigeria")
        save_homework_code(
            "TCH001",
            "thread_x",
            "raw",
            [{"question": "Q", "options": ["A", "B", "C", "D"], "correct": 0}],
            "Title: Hw\nDetails: Body",
            teacher_phone="+23412345678",
        )

        response = client.get("/api/h/TCH001")
        assert response.status_code == 200
        data = response.json()
        assert "teacher" in data
        assert data["teacher"]["name"] == "Mrs. Okafor"
        assert data["teacher"]["slug"]  # generated by save_teacher

    def test_response_omits_teacher_when_no_teacher_phone(self):
        """Legacy/anonymous homework codes (no teacher_phone) — the
        `teacher` key is absent so frontend never renders a stale link."""
        save_homework_code(
            "ANON01",
            "thread_anon",
            "raw",
        ...
        response = client.get("/api/h/ANON01")
        assert response.status_code == 200
        assert "teacher" not in response.json()

    def test_response_omits_teacher_when_phone_not_registered(self):
        """Defensive: teacher_phone is set but no teacher row exists.
        Frontend must not see a half-populated teacher object."""
        save_homework_code(
            "ORPH01",
            "thread_orphan",
            "raw",
            [{"question": "Q", "options": ["A", "B", "C", "D"], "correct": 0}],
            "Title: Hw\nDetails: Body",
            teacher_phone="+23499999999",
        )

        response = client.get("/api/h/ORPH01")
        assert response.status_code == 200
        assert "teacher" not in response.json()
