...
def get_weekly_usage(teacher_phone: str) -> int:
    ...
    if not supabase:
        ...
        return sum(
            1
            for e in events
            if e.get("action") == "lesson" and (e.get("created_at") or "") >= week_start_iso
        )
    try:
        response = (
...
def save_subscription(
    teacher_phone: str, tier: str, payment_ref: str = "", school_phone: str = ""
) -> bool:
    ...
    if not supabase:
        ...
        return True
    try:
        supabase.table("subscriptions").upsert(record, on_conflict="teacher_phone").execute()
        return True
    except Exception as e:
